...
function get_vₜ_averaged_radius_collision_velocity_parameterisation_num_2(indx, Rindx)
...
function get_vₜ_averaged_radius_collision_velocity_parameterisation_num_13(indx, Rindx)
    return sqrt(ϵ/(15ν))*(effective_areas[indx, Rindx]) # spherical approximation
end

function get_vₜ_same_radius_averaged_radius_collision_velocity_parameterisation_num_2(indx)
...
function get_vₜ_same_radius_averaged_radius_collision_velocity_parameterisation_num_13(indx)
...
function get_Fenc_average_radius_collision_velocity_parameterisation_num_3(indx, Rindx, nRindx, vcoll)
    if indx == Rindx
        Fenc =  π*(effective_areas[indx, Rindx])^2 * nRindx * vcoll
    else
        Fenc = 2*π*(effective_areas[indx, Rindx])^2 * nRindx * vcoll
    end
    return Fenc
end
...
function get_Fenc_average_radius_collision_velocity_parameterisation_num_12(indx, Rindx, nRindx, vcoll)
    if indx == Rindx
        Fenc =  π * (effective_areas[indx, Rindx])^2 * nRindx/2 * vcoll
    else
        Fenc = π * (effective_areas[Rindx, Rindx])^2 * nRindx * vcoll
    end
    return Fenc
end
...
function get_Fenc_p1_average_radius_collision_velocity_parameterisation_num_3(indx, vcoll, ntot)
    #Fenc =  (3/(2*aspect_ratio))^(2/3) * 2*π*(Rs[indx])^2 * vcoll * ntot
    return  2*π*(effective_areas[indx, indx]/2)^2 * vcoll * ntot
end
...
function get_Fenc_p1_average_radius_collision_velocity_parameterisation_num_12(indx, vcoll, ntot)
    ...
    return π * (effective_areas[indx, indx]/2)^2  * vcoll * ntot
end
...
simulation = Simulation(model, Δt=10.0, stop_time=4000minutes)
