        ...
        if concentration_parameterisation_num == 2
            ...
            for Rindx in eachindex(Rs)
                ...
                if collision_velocity_parameterisation_num == 3
                    if indx == Rindx
                        if effective_radius
                            Fenc .+= (3/(2*aspect_ratio))^(2/3)*2*π*(Rs[indx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        else
                            Fenc .+= 2*π*(Rs[indx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        end
                    else
                        if effective_radius
        ...
        for jindx in 2:lastindex(Rs)
            ...
            for Rindx in eachindex(Rs)
                ...
                if collision_velocity_parameterisation_num == 3
                    if Rindx == jindx
                        if effective_radius
                            Fenc .+=  (3/(2*aspect_ratio))^(2/3) * 2*π*(Rs[jindx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        else
                            Fenc .+= 2*π*(Rs[jindx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        end
                    else
                        if effective_radius
                            Fenc .+=  (3/(2*aspect_ratio))^(2/3) * 2*π*(Rs[jindx] + Rs[Rindx])^2*nRindx * vcoll
                        else
                            Fenc .+= 2*π*(Rs[jindx] + Rs[Rindx])^2*nRindx * vcoll
                        end
                    end
                    ...
                    if Rindx == jindx
                        if effective_radius
                            Fenc .+= (3/(2*aspect_ratio))^(2/3) *  π*(Rs[jindx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        else
                            Fenc .+= π*(Rs[jindx] + Rs[Rindx])^2*nRindx/2 * vcoll
                        end
                    else
                        if effective_radius
                            Fenc .+= (3/(2*aspect_ratio))^(2/3) *  π*(Rs[jindx] + Rs[Rindx])^2*nRindx * vcoll
                        else
                            Fenc .+= π*(Rs[jindx] + Rs[Rindx])^2*nRindx * vcoll
                        end
                    end
                end
            end  
...
simulation = Simulation(model, Δt=100000.0, stop_time=4000hours)
...
conjure_time_step_wizard!(simulation, cfl=1.0, max_Δt=100000minute)#0.01minute)
