...
#for pnum = [1, 2]
end_name = "fast_same_n_just_collisions_epsilon" * string(ϵ) * "_" * string(numSizeClasses) * "_depth_" * string(depth) * "T"
...
function nend_forcing_func(i, j, k, grid, clock, model_fields, indx)
    ...
    if Gₙ₋₁[i, j, k] > 0 # growth
        ...
        return @inbounds Gₙ₋₁[i, j, k]*nₙ₋₁[i, j, k]/(Vᵢ - Vₙ₋₁) + dn_coll[i, j, k] + udn_dz[i, j, k]
    else # melt
        final_conc = (Gₙ[i, j, k]*nᵢ[i, j, k])/(Vᵢ - Vₙ₋₁) + dn_coll[i, j, k] + udn_dz[i, j, k]
...
function nintermediate_forcing_func(i, j, k, grid, clock, model_fields, indx)
    ...
    if G_im1[i, j, k] > 0  # Growth case
        return @inbounds - (Gᵢ[i, j, k] * nᵢ[i, j, k] / (V_ip1 - Vᵢ) - G_im1[i, j, k] * n_im1[i, j, k] / (Vᵢ - V_im1)) + dn_coll[i, j, k] + udn_dz[i, j, k]
    else  # Melt case
        return @inbounds - (G_ip1[i, j, k] * n_ip1[i, j, k] / (V_ip1 - Vᵢ) - Gᵢ[i, j, k] * nᵢ[i, j, k] / (Vᵢ - V_im1)) + dn_coll[i, j, k] + udn_dz[i, j, k]
    end
end
...
function n1_forcing_func(i, j, k, grid, clock, model_fields, indx)
    ...
    if G₁[i, j, k] > 0 # growth
        return @inbounds  - G₁[i, j, k]*model_fields.n1[i, j, k]/(V₂ - V₁) + dn_coll[i, j, k] + udn_dz[i, j, k]
    else # melt
        return @inbounds   - (G₂[i, j, k]*model_fields.n2[i, j, k]/(V₂ - V₁) - G₁[i, j, k]*model_fields.n1[i, j, k]/V₁) + dn_coll[i, j, k] + udn_dz[i, j, k]
    end
end
