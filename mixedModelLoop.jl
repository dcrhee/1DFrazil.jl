    ...
    if αs[indx] == 0
        ...
        if concentration_parameterisation_num == 2
            ...
            for Rindx in eachindex(Rs)
                ...
                vcoll = find_vcoll(uRindx - uᵢ, vₜ, collision_velocity_parameterisation_num)
            ...
            vcoll = find_vcoll(uᵢ, vₜ, collision_velocity_parameterisation_num)
        ...
        if β == 0
            ...
            if concentration_parameterisation_num == 2
                ...
                for Rindx in eachindex(Rs)
                    ...
                    vcoll = find_vcoll(uRindx - uᵦ, vₜ, collision_velocity_parameterisation_num) 
                    if collision_velocity_parameterisation_num == 3
                        if effective_radius
                ...
                vcoll = find_vcoll(uᵦ, vₜ, collision_velocity_parameterisation_num)
                nTotal = zeros(1, 1, numz)
                for Rindx in eachindex(Rs)
                    nRindx = getfield(model_fields, Symbol("n$Rindx"))
                    nTotal .+= nRindx
    ...
    if concentration_parameterisation_num == 2
        ...
        for jindx in 2:lastindex(Rs)
            ...
            for Rindx in eachindex(Rs)
                ...
                vcoll = find_vcoll(uRindx - uⱼ, vₜ, collision_velocity_parameterisation_num)
                if collision_velocity_parameterisation_num == 3
                    if effective_radius
                        Fenc .+=  (3/(2*aspect_ratio))^(2/3) * 2*π*(Rs[indx] + Rs[Rindx])^2*nRindx * vcoll
                    else
        ...
        for jindx in 2:lastindex(Rs)
            ...
            vcoll = find_vcoll(uⱼ, vₜ, collision_velocity_parameterisation_num)
            nTotal = zeros(1, 1, numz)
            for Rindx in eachindex(Rs)
                nRindx = getfield(model_fields, Symbol("n$Rindx"))
                nTotal .+= nRindx
...
#for cvelnum = [1, 2, 3]
    #for pnum = [1, 2]
        end_name = "match_Feltham_no_n_max_eps_0.01_depth_10"
        ...
        simulation = Simulation(model, Δt=1.0, stop_time=144hours)
