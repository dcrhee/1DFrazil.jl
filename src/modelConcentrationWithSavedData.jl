...
include("loadLastSnapshot.jl")
using .loadLastSnapshot
...
old_data_name = end_name * "_v4"
end_name = end_name * "_v5"
...
nᵢₙ = load_data("1D_fields" * old_data_name * ".jld2", numSizeClasses) #6429774.231059961/length(Rs) * ones(length(Rs))
...
simulation = Simulation(model, Δt=1.0, stop_time=8000000minutes)
...
conjure_time_step_wizard!(simulation, cfl=1.0, max_Δt= 96hours) # for n = 1, p = 2

#simulation.callbacks[:progress] = Callback(progress, IterationInterval(20))

output_interval = 96hours
