...
ts_1 = load_plot_data("1D_fieldsnew_v_log_space_same_C_just_collisions_epsilon1.0e-8_200.jld2", numSizeClasses)
#ts_1 = load_plot_data("1D_fieldsnew_v_same_C_just_collisions_epsilon1.0e-8_200.jld2", numSizeClasses)
...
#ts_1 = load_plot_data("1D_fieldssameConc_fast_same_C_just_collisions_epsilon0.01_200_new_spherical_sum_nj.jld2", numSizeClasses)
#ts_1 = load_plot_data("1D_fieldslog_space_sameConc_fast_same_C_just_collisions_epsilon1.0e-8_200_new_spherical_sum_nj.jld2", numSizeClasses)
#ts_1 = load_plot_data("1D_fieldslog_space_sameConc_fast_same_C_just_collisions_epsilon0.01_200_new_spherical_sum_nj.jld2", numSizeClasses)
#ts_1 = load_plot_data("1D_fieldssameConc_fast_same_C_just_collisions_epsilon1.0e-8_200_new_spherical_sum_nj.jld2", numSizeClasses)
#ts_1 = ts_nonsum
...
#ts_nonsum = load_plot_data("1D_fieldsnew_v_same_C_just_collisions_epsilon1.0e-8_200_sum_nj.jld2", numSizeClasses)
ts_nonsum = load_plot_data("1D_fieldsnew_v_log_space_same_C_just_collisions_epsilon1.0e-8_200_new_spherical_sum_nj.jld2", numSizeClasses)
...
#ts_nonsum = load_plot_data("1D_fieldssameConc_fast_same_C_just_collisions_epsilon1.0e-8_200_new_spherical_sum_nj_redistribute.jld2", numSizeClasses)
#ts_nonsum = load_plot_data("1D_fieldssameConc_fast_same_C_just_collisions_epsilon0.01_200_new_spherical_sum_nj_redistribute.jld2", numSizeClasses)

#ts_nonsum = load_plot_data("1D_fieldslog_space_sameConc_fast_same_C_just_collisions_epsilon0.01_200_new_spherical_sum_nj_redistribute.jld2", numSizeClasses)
#ts_nonsum = load_plot_data("1D_fieldslog_space_sameConc_fast_same_C_just_collisions_epsilon1.0e-8_200_new_spherical_sum_nj_redistribute.jld2", numSizeClasses)
...
function plot_n_C_pdf(ls, time_series_data, times, add_nC_label, add_pdf_label, end_time_series_data)
    ...
    for (i, n1val) in enumerate(fractions*n1max)
        ...
        ns = find_z_mean(time_series_data.n1)
        if ls == 1
        ...
        elseif tindx == 1 #change to == 1
            ...
            if ls == 1
                if add_pdf_label
                    ...
                    lines!(ax_rn, Rs[2:end], barvals/(sum(barvals)*(Rs[1])), linestyle=:solid,  color = colours2[i], label = label_str)
                else
                    ...
                    lines!(ax_rn, Rs[2:end], barvals/(sum(barvals)*(Rs[1])), linestyle=:solid,  color = colours2[i])
                end
            elseif ls == 2
                if add_pdf_label
                    ...
                    lines!(ax_rn, Rs[2:end], barvals/(sum(barvals)*(Rs[1])), linestyle=:dash,  color = colours2[i], label = label_str)
                else
                    ...
                    lines!(ax_rn, Rs[2:end], barvals/(sum(barvals)*(Rs[1])), linestyle=:dash,  color = colours2[i])
                end
            end
        end
...
ax_rn = Axis(fig[1:4, 3];
            ...
            xscale = log10,
            #yscale = log10, #)
            #limits = ((Rs[1]*1000, Rs[end]*1000), (0, 5e3)))
            limits = ((Rs[1], Rs[end]), (1e-0, 1e3)))
...
#axislegend(ax_rn, position=(:right, :top), framevisible = false, nbanks = 2)
...
#save("Cdot_interact_eps1e-8.pdf", fig)
#save("Csum_redistribute_eps1e-8.pdf", fig)
#save("Csum_redistribute_eps0.01_c3.pdf", fig)

#save("logCsum_redistribute_eps0.01_c3.pdf", fig)
#save("logCsum_redistribute_eps1e-8_c3.pdf", fig)
#save("logCdot_interact_0.01_c13.pdf", fig)
#save("logCdot_interact_eps1e-8.pdf", fig)

#save("Cnewdot_eps1e-8.pdf", fig)
save("logCnewdot_eps1e-8.pdf", fig)
