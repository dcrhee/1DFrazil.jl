...
#feltham_name = "fast_same_C_just_collisions_epsilon" * string(ϵ) * "_" * string(numSizeClasses)#"_same_n_just_collisions_epsilon" * string(ϵ)
#feltham_name = "fast_same_C_just_collisions_epsilon" * string(ϵ) * "_" * string(numSizeClasses) * "_depth_" * string(depth)
feltham_name = "fast_same_C_just_collisions_epsilon" * string(ϵ) * "_" * string(numSizeClasses)  * "_sum_nj"
...
function generatePDF(tindx, chosentimeseries)
    ...
    for i in 2:numSizeClasses
        Civals = find_C_const(i)*find_z_mean(getfield(chosentimeseries, Symbol("n$i")))/Cᵢₙ  # Extract tracer values
        append!(expanded_data, Rs[i] * ones(round(Int, Civals[tindx])))
        barvals[i-1] = Civals[tindx]
    end
...
for (i, n1val) in enumerate(exp10.(range(start=log10(n1max/10), stop=log10(n1max/1.001), length=numpdflines)))
    ...
    pdf, barvals = generatePDF(tindx, time_series)
    lines!(ax_r, pdf.x*1000, pdf.density, linestyle=:solid,  color = colours[round(Int, numSizeClasses/numpdflines*i)], label = "t = " * string(feltham_times[tindx]) * " days")
end
...
for (i, n1val) in enumerate(exp10.(range(start=log10(n1max/10), stop=log10(n1max/1.001), length=numpdflines)))
    ...
    pdf, barvals = generatePDF(tindx, time_series)
    lines!(ax_rt, pdf.x*1000, pdf.density, linestyle=:solid,  color = colours[round(Int, numSizeClasses/numpdflines*i)], label = "t = " * string(feltham_times[tindx]) * " days")
end
